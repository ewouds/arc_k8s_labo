...
if ($existing -eq "Failed") {
  Write-Host "  🗑️  Removing previous failed installation..." -ForegroundColor DarkYellow
...
Write-Host ""
Write-Host "⏳ Waiting for extension to provision (checking every 30s, max 10 min)..." -ForegroundColor DarkYellow
$maxAttempts = 20
$attempt = 0
$state = "Creating"
...
if ($state -eq "Failed") {
  Write-Host ""
  Write-Host "  ⚠️  Extension provisioning failed (common on small VMs)." -ForegroundColor DarkYellow
...
elseif ($state -eq "Succeeded") {
  ...
  Write-Host ""
  Write-Host "  ⏳ Extension still provisioning after 10 min." -ForegroundColor DarkYellow
...
Write-Host ""
Write-Host "🔍 Verifying Defender pods on the cluster..." -ForegroundColor Yellow
...
if ($mdcPods -ne 0) {
  Write-Host "  ⚠️  No Defender pods found yet (namespace 'mdc' may take a few minutes)" -ForegroundColor DarkYellow
}
else {
  Write-Host "  ✅ Defender sensor pods are running" -ForegroundColor Green
}

# --- 4. Trigger a test security alert ---
Write-Host ""
Write-Host "🧪 Triggering a test security alert..." -ForegroundColor Yellow
Write-Host "  Running the official Microsoft Defender test alert container" -ForegroundColor DarkGray
Write-Host ""
Write-Host "  ⚠️  If governance policies (Section 5) are active, disable them first:" -ForegroundColor DarkYellow
...
if ($LASTEXITCODE -eq 0) {
  Write-Host "  ✅ Test alert triggered — will appear in Defender for Cloud within ~30 min" -ForegroundColor Green
}
else {
  Write-Host "  ⚠️  Could not trigger test alert (check SSH connectivity)" -ForegroundColor DarkYellow
}

# --- 5. Portal walkthrough ---
Write-Host ""
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor DarkGray
Write-Host "  📊 Portal Demo Walkthrough:" -ForegroundColor White
Write-Host ""
...
Write-Host ""
Write-Host "🧹 Cleanup (run after demo):" -ForegroundColor Yellow
